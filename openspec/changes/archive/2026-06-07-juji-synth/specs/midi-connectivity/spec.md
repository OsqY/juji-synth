...
The synthesizer SHALL detect USB MIDI devices connected to the Android device.

#### Scenario: MIDI device connected
- **WHEN** user connects a USB MIDI controller to the Android device
- **THEN** the device appears in the MIDI device list within the app
...
The synthesizer SHALL remember connected MIDI devices and automatically reconnect.

#### Scenario: Device reconnection
- **WHEN** previously used MIDI device is connected again
- **THEN** app automatically recognizes and enables MIDI input from that device