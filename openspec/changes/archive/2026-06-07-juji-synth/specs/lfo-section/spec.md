...
The synthesizer SHALL provide two independent Low Frequency Oscillators (LFO1 and LFO2) for creating cyclic modulation effects.
...
Each LFO SHALL support the following waveform types: sine, square, sawtooth, triangle, random (sample-and-hold).
...
Each LFO SHALL have rate control adjustable from 0.01Hz to 50Hz controlling modulation speed.
...
Each LFO SHALL have depth control adjustable from 0% to 100% controlling modulation intensity.
...
### Requirement: LFO destinations
Each LFO SHALL be routable to multiple destination parameters including pitch (vibrato), filter cutoff (wobble), amplitude (tremolo), and oscillator mix.

#### Scenario: LFO modulates filter
- **WHEN** LFO is routed to filter cutoff with depth at 50%
- **THEN** filter cutoff modulates up and down in sync with LFO rate

#### Scenario: LFO modulates pitch
- **WHEN** LFO is routed to pitch with depth at 25%
- **THEN** pitch varies slightly above and below played note following LFO shape